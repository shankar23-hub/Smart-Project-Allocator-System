...
export default function MyProfile({ user }) {
  ...
  const stats = [
    { label:'Staff Managed',    value: 44, icon:'👥', color:'#ef4d6a' },
    { label:'Projects Overseen', value: 12, icon:'📁', color:'#4cc9f0' },
    { label:'IDs Generated',    value: 38, icon:'🪪', color:'#7c5cff' },
    ...
    <div>
      ...
      {toast && (
        <div style={{ position:'fixed',top:24,right:24,zIndex:9999,
          ...
          background:toast.type==='error'?'#ef4d6a':'#06d6a0',color:'#fff',
      ...
      <div style={{ background:'linear-gradient(135deg,#0d0f18,#161924,#1a1f2e)',border:'1px solid var(--panel-border)',borderRadius:'var(--radius-xl)',padding:32,marginBottom:24,position:'relative',overflow:'hidden' }}>
        <div style={{ position:'absolute',top:-60,right:-60,width:240,height:240,borderRadius:'50%',background:'rgba(239,77,106,0.07)' }}></div>
        ...
        <div className="card" style={{ padding:28 }}>
          ...
          <div style={{ maxWidth:420 }}>
            ...
            <div className="form-group">
              ...
              {pwForm.next && pwForm.next.length < 6 && <div style={{ fontSize:11,color:'#ef4d6a',marginTop:4 }}>Password must be at least 6 characters</div>}
            </div>
            <div className="form-group">
              ...
              {pwForm.confirm && pwForm.next !== pwForm.confirm && <div style={{ fontSize:11,color:'#ef4d6a',marginTop:4 }}>Passwords do not match</div>}
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
            {activityLog.map((a,i)=>(
              <div key={i} style={{ display:'flex',alignItems:'center',gap:14,padding:'12px 16px',background:'var(--bg3)',borderRadius:10,border:'1px solid var(--panel-border)' }}>
                <div style={{ width:38,height:38,borderRadius:10,background:'rgba(239,77,106,0.1)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0 }}>{a.icon}</div>
                <div style={{ flex:1 }}>
