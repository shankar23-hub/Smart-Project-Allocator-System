...
// Vercel serverless request body limit is small. Never send huge base64 images to the API.
// Staff photos are stored only as compressed thumbnails. Oversized old images are removed.
const MAX_IMAGE_DATA_URL_CHARS = 260000; // ~190 KB binary after base64
...
function safeImageDataUrl(value) {
  ...
  return value.length <= MAX_IMAGE_DATA_URL_CHARS ? value : "";
}
...
function stripLargeImages(value) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (["image", "file", "photoFile", "attachment"].includes(key)) continue;
    if (["imagePreview", "profileImage", "photo", "avatarImage"].includes(key)) {
      output[key] = safeImageDataUrl(item);
    } else {
      output[key] = stripLargeImages(item);
    }
  }
  return output;
}
...
function jsonBody(payload) {
  return JSON.stringify(stripLargeImages(payload || {}));
}

function sanitizeEmployeePayload(emp = {}) {
  const clean = stripLargeImages(emp);
  clean.imagePreview = safeImageDataUrl(clean.imagePreview || clean.profileImage || "");
  clean.profileImage = clean.imagePreview;
  return clean;
}
...
async function apiFetch(path, options = {}) {
  ...
  if (!res.ok) {
    const msg =
      ...
      body?.error ||
      body?.detail ||
      (res.status === 413 ? "Request Entity Too Large. Please remove or re-upload the staff photo; it will be compressed automatically." : `HTTP ${res.status}`);
...
function normalizeEmployee(emp = {}) {
  return {
    ...
    city: emp.city || "",
    imagePreview: safeImageDataUrl(emp.imagePreview || emp.profileImage || ""),
    profileImage: safeImageDataUrl(emp.profileImage || emp.imagePreview || ""),
  };
}
...
export const employeeAPI = {
  ...
  async create(payload) {
    const data = await apiFetch("/api/employees", {
      method: "POST",
      body: jsonBody(sanitizeEmployeePayload(normalizeEmployee(payload))),
    });
...
export const projectAPI = {
  ...
  async create(payload) {
    const data = await apiFetch("/api/projects", {
      method: "POST",
      body: jsonBody(normalizeProject(payload)),
    });
